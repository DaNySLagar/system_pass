<x-app-layout>
    ...
    <div class="overflow-x-auto flex-col bg-white rounded-lg  p-2 m-2 w-full ">
        ...
        <h1 class="font-bold text-center text-3xl mb-3">Lista de Cargos del Gobierno Regional</h1>
        ...
        @endif
            ...
            <form action="charges" method="POST" class="bg-white flex space-x-1 pb-2 w-full h-12 ">
                @csrf
                <div class='bg-yellow-400  w-1/2 md:w-1/5 '>
                    <input type="text" name="name_charge" class="rounded h-full w-full" placeholder="Nuevo Cargo">
                </div>
                ...
                <tbody>
                    @forelse($charges as $charge)
                        <tr class="bg-white border-b bg-white-800">
                            <td class="p-2 border border-gray-300">{{ $charge->name_charge }}</td>
                            <td class="p-2 border border-gray-300">
                                <div class="w-full h-full place-content-around flex  flex-col md:flex-row  justify-center items-center align-middle">
                                    ...
                                    <form action="charges/{{ $charge->id }}" method="POST" id="deleteForm" class="deleteForm{{$charge->id}}">
                                        @csrf
                                        @method('DELETE')
                                        <input type="button" class="bg-red-500 text-white rounded px-2 py-1 mx-1"
                                            value="Eliminar" onclick="mostrarConfirmacionform({{$charge->id}})">
                                    </form>
...
function mostrarConfirmacionform(chargeId) {
        Swal.fire({
            title: '¿Estás seguro?',
            ...
            if (result.isConfirmed) {
                var form = document.querySelector('.deleteForm'+chargeId);
                form.submit();
            }
        });
    }
