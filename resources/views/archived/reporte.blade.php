...
<body>
    <div id="header_body_report">
        ...
        <div class="infoheader_body">
            ...
            <h4>REPORTE DE PAPAPELETAS DE SALIDA ARCHIVADAS</h4>
        </div>
    </div>
    ...
    <div>
        <table class="tabla">
            <thead>
                <tr>
                    <th scope="col" class="encabezado py-2">DNI</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if (isset($pass->departure_time) && isset($pass->departure_time->hour_departure))
                            <td class="fila py-2">{{ $pass->departure_time->hour_departure }}</td>
                        @else
                            <td class="fila py-2">Sin marcar hora de salida</td>
                        @endif

                        @if (isset($pass->return_time) && isset($pass->return_time->hour_return))
                            <td class="fila py-2">{{ $pass->return_time->hour_return }}</td>
                        @else
                            <td class="fila py-2">Sin marcar hora de retorno</td>
                        @endif

                        @if (isset($pass->return_time) && isset($pass->return_time->observation))
                            <td class="fila py-2">{{ $pass->return_time->observation }}</td>
                        @else
                            <td class="fila py-2">Sin Observaciones </td>
                        @endif
